 ...
 * @author Admin
 */
@WebServlet(name = "TransServlet", urlPatterns = {"/TransServlet"})
public class TransServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String option = request.getParameter("group1");
        session.setAttribute("dbTable", request.getParameter("dbTable"));
        
        switch(option){
            case "searchby":
                response.sendRedirect("searchby.html");
                break;
            case "select":
